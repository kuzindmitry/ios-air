...
- (void)createContentDataArray {
    NSArray *titles = [NSArray arrayWithObjects:@"about_app_header", @"tickets_header", @"map_price_header", @"favorites_header", nil];
    NSArray *contents = [NSArray arrayWithObjects:@"about_app_describe", @"tickets_describe", @"map_price_describe", @"favorites_describe", nil];
    for (int i = 0; i < 4; ++i) {
        contentData[i].title = ((NSString *)[titles objectAtIndex:i]).localize;
        contentData[i].contentText = ((NSString *)[contents objectAtIndex:i]).localize;
...
- (void)updateButtonWithIndex:(int)index {
    ...
    switch (index) {
        ...
        case 2:
            title = @"next_button";
            [_nextButton setTitle:title.localize forState:UIControlStateNormal];
            _nextButton.tag = 0;
            break;
        case 3:
            title = @"done_button";
